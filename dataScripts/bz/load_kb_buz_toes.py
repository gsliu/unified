...
##############################################################################
    ...
    def index_item(self, item):
        doc = {
            ...
            'fixby': item[10],
            'text': item[11],
	    'url': 'https://bugzilla.eng.vmware.com/show_bug.cgi?id=' + str(item[0])
        }
        #print doc
    ...
    def index_all(self):
        ...
        # iter all
        sql = 'select distinct bug_id from bug_kb_map'
        bz_cur.execute(sql)
        buglist = bz_cur.fetchall()

        for bz in buglist:
            print 'indexing...%s' % bz
            tmp = self.create_item(bz, bz_con, bz_cur)
            self.index_item(tmp)
